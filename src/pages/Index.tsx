  ...
  return <>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <motion.img src={heroImage} alt="Robotic spray coating workstation" className="w-full h-full object-cover" initial={{
          scale: 1.1
        }} animate={{
          scale: 1
        }} transition={{
        ...
        <div className="container-wide relative py-20 md:py-32">
          <div className="max-w-3xl">
            <motion.h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-primary-foreground mb-6 leading-tight" initial={{
            opacity: 0,
            y: 30
          }} animate={{
            opacity: 1,
            ...
            <motion.p className="text-xl text-primary-foreground/80 mb-8 leading-relaxed max-w-2xl" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.6
              ...
              <Button asChild size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground font-semibold text-lg h-14 px-8 transition-transform hover:scale-105">
                <Link to="/quote" className="flex items-center gap-2">
                  Configure Paint Cell
                  <ChevronRight className="h-5 w-5" />
                </Link>
              </Button>
              <Button asChild variant="outline" size="lg" className="border-white/40 bg-white/10 text-white hover:bg-white/20 h-14 px-8 transition-transform hover:scale-105">
                <Link to="/paint-cells">
                  Explore Solutions
                </Link>
              </Button>
      ...
      <Section variant="default">
        ...
        <StaggerContainer className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {benefits.map((benefit: Benefit) => <StaggerItem key={benefit.title}>
              <motion.div className="bg-card rounded-xl p-6 border border-border card-elevated h-full flex flex-col cursor-pointer" whileHover={{
            y: -5,
            transition: {
              duration: 0.2
            }
          }} onClick={() => handleCardClick(benefit)} role="button" tabIndex={0} onKeyDown={e => {
      ...
      <Section variant="muted">
        ...
        <StaggerContainer className="grid md:grid-cols-2 gap-6">
          {systemComponents.map(component => <StaggerItem key={component.title}>
              <motion.div className="bg-card rounded-xl p-6 border border-border flex gap-4 h-full" whileHover={{
            y: -3,
            transition: {
              duration: 0.2
            }
      ...
      <Section variant="primary">
        <FadeIn className="text-center max-w-3xl mx-auto">
          ...
          <p className="text-lg text-primary-foreground/80 mb-8">Our guided configurator helps you define your requirements in minutes. 
Get a tailored solution proposal based on your specific needs.</p>
          <Button asChild size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground font-semibold text-lg h-14 px-8 transition-transform hover:scale-105">
            <Link to="/quote" className="flex items-center gap-2">
              Start Assessment
              <ChevronRight className="h-5 w-5" />
